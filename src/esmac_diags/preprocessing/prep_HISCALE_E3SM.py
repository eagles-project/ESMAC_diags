        ...
        if len(file_inds) > 1:
            for ii in np.arange(len(file_inds)-1):
                ...
                if config['ccn_output'] == True:
                    ...
                    if len(matched_vlist) == len(req_vlist):
                        print('\nAnalyzing CCN')
                        new_ccn1 = e3smdata3d_wetaer[config['num_a1']+E3SMdomain_range][:,:,latlon_ind,...].load()
                        new_ccn2 = e3smdata3d_wetaer[config['num_a2']+E3SMdomain_range][:,:,latlon_ind,...].load()
                        new_ccn5 = e3smdata3d_wetaer[config['num_a3']+E3SMdomain_range][:,:,latlon_ind,...].load()
                        new_ccn10 = e3smdata3d_wetaer[config['num_a4']+E3SMdomain_range][:,:,latlon_ind,...].load()
