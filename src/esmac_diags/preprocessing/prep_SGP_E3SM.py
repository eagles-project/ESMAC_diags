...
def prep_E3SM_sfc(input_path, input2d_filehead, input3d_filehead, input3d_dryaerosol_filehead, input3d_cloudaerosol_filehead, input_cosp_filehead, output_path, output_filehead, dt=3600, config=config):
    ...
    # lst.sort()
    lst3d = glob.glob(input_path + input3d_filehead+'.*20??-??-??-*.nc')
    lst3d_dryaer = glob.glob(input_path + input3d_dryaerosol_filehead+'.*20??-??-??-*.nc')
    lst3d_wetaer = glob.glob(input_path + input3d_cloudaerosol_filehead+'.*20??-??-??-*.nc')
    lst2d = glob.glob(input_path + input2d_filehead+'.*20??-??-??-*.nc')
    lstcosp = glob.glob(input_path + input_cosp_filehead+'.*20??-??-??-*.nc')
    lst3d.sort()
    lst3d_dryaer.sort()
    lst3d_wetaer.sort()
    lst2d.sort()
    ...
    # mean cloud droplet number concentration, COD, and Reff
