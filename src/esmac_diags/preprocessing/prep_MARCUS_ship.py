...
def prep_CCN(shipmetpath, ccnpath, prep_data_path, dt=3600):
    ...
    time_new = pd.date_range(start='2017-10-21 4:00:00', end='2018-03-23 23:59:00', freq=str(int(dt))+"s")  # MARCUS time period
...
exhaustfreepath : str
    ...
    #%% re-shape the data into coarser resolution
    time_new = pd.date_range(start='2017-10-29 4:00:00', end='2018-03-23 23:59:00', freq=str(int(dt))+"s")  # MARCUS time period
...
def prep_CNsize_exhaustfree(shipmetpath, exhaustfreepath, prep_data_path, dt=3600):
    ...
    tmpuhsas = xr.DataArray(data=np.array(uhsas), dims=["time", "size"], coords=dict(time=time1, size=size))
