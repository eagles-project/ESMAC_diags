...
def prep_E3SM_sfc(input_path, input2d_filehead, input3d_filehead, input3d_dryaerosol_filehead, input3d_cloudaerosol_filehead, input_cosp_filehead, output_path, output_filehead, dt=3600, config=config):
    ...
    # lst.sort()
    lst3d = glob.glob(input_path + input3d_filehead+'.*2017-06-2?-*.nc') + \
            glob.glob(input_path + input3d_filehead+'.*2017-07-0?-*.nc') + \
            glob.glob(input_path + input3d_filehead+'.*2018-01-??-*.nc') + \
            glob.glob(input_path + input3d_filehead+'.*2018-02-??-*.nc')
    lst3d_dryaer = glob.glob(input_path + input3d_dryaerosol_filehead+'.*2017-06-2?-*.nc') + \
            glob.glob(input_path + input3d_dryaerosol_filehead+'.*2017-07-0?-*.nc') + \
            glob.glob(input_path + input3d_dryaerosol_filehead+'.*2018-01-??-*.nc') + \
            glob.glob(input_path + input3d_dryaerosol_filehead+'.*2018-02-??-*.nc')
    lst3d_wetaer = glob.glob(input_path + input3d_cloudaerosol_filehead+'.*2017-06-2?-*.nc') + \
            glob.glob(input_path + input3d_cloudaerosol_filehead+'.*2017-07-0?-*.nc') + \
    ...
    lst2d = glob.glob(input_path + input2d_filehead+'.*2017-06-2?-*.nc') + \
            glob.glob(input_path + input2d_filehead+'.*2017-07-0?-*.nc') + \
            glob.glob(input_path + input2d_filehead+'.*2018-01-??-*.nc') + \
            glob.glob(input_path + input2d_filehead+'.*2018-02-??-*.nc')
    lstcosp = glob.glob(input_path + input_cosp_filehead+'.*2017-06-19-82800.nc') + \
            glob.glob(input_path + input_cosp_filehead+'.*2017-06-2?-*.nc') + \
            glob.glob(input_path + input_cosp_filehead+'.*2017-07-0?-*.nc') + \
    ...
    for varname in variablecosp_names:
        try:
            ...
            if varname == config['IWPMODIS'] + E3SMdomain_range or varname == config['REFFICEMODIS'] + E3SMdomain_range or varname == config['TAUICEMODIS'] + E3SMdomain_range:
                var[:] = var/e3smdatacosp[config['CFICEMODIS'] + E3SMdomain_range][:,x_idx].values
            if varname == config['TAUTOTMODIS'] + E3SMdomain_range:
                var[:] = var/e3smdatacosp[config['CFTOTMODIS'] + E3SMdomain_range][:,x_idx].values
            var.coords[config['time_dim']] = var.indexes[config['time_dim']].to_datetimeindex() # change time to standard datetime64 format
        except:
            # var = xr.DataArray(np.zeros((len(e3smtime),len_ncol))*np.nan,name=varname,\
            #                    dims=[config['time_dim'],config['latlon_dim']+E3SMdomain_range],coords={config['time_dim']:e3smtime,config['latlon_dim']+E3SMdomain_range:np.arange(len_ncol)},\
