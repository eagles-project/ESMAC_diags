...
def run_plot(settings):
    ...
    if campaign=='MAGIC':
        ...
        (time, lat, timeunit, latunit, lat_longname) = read_ship_exhaustfree(shipccn_exhaustfree_path + \
                                                     'CCN_exhaustfree_1hr.nc', 'lat')
        (time, lon, timeunit, lonunit, lon_longname) = read_ship_exhaustfree(shipccn_exhaustfree_path + \
                                                     'CCN_exhaustfree_1hr.nc', 'lon')
        (time, ccn1, timeunit, ccnunit, ccn_longname) = read_ship_exhaustfree(shipccn_exhaustfree_path + \
                                                     'CCN_exhaustfree_1hr.nc', 'CCN1')
        (time, ccn5, timeunit, ccnunit, ccn_longname) = read_ship_exhaustfree(shipccn_exhaustfree_path + \
                                                     'CCN_exhaustfree_1hr.nc', 'CCN5')
        ...
        for bb in range(latlen):
            ...
            ccn3_o.append(ccn1[idx])
            ccn5_o.append(ccn5[idx])
