"""
script to generate all plots for SOCRATES aircraft data
...
lst = sorted(glob.glob(prep_obs_path + 'CCN_'+site+'_*.nc'))
obsdata = xr.open_mfdataset(lst)
time_socrates = obsdata['time'].load()
ccn2 = obsdata['CCN2_spec'].load()
ccn2_scan = obsdata['CCN2_scan'].load()
obsdata.close()
...
obsdata = xr.open_mfdataset(lst)
time_socrates = obsdata['time'].load()
size_2ds = obsdata['size_2ds'].load()
size_cdp = obsdata['size_cdp'].load()
nd_bin_2ds = obsdata['Nd_2ds'].load()
...
nd_bin_2ds = nd_bin_2ds/1000 # #/L to #/cm3

# calculate effective radius from observed size distribution
reff = (np.sum(nd_bin_cdp * (size_cdp**3), axis=1) + np.sum(nd_bin_2ds * (size_2ds**3), axis=1)) / \
        (np.sum(nd_bin_cdp * (size_cdp**2), axis=1) + np.sum(nd_bin_2ds * (size_2ds**2), axis=1))
reff[reff>50] = np.nan
    
# remove Nd less than 10 cm-3
nd_all[nd_all<10] = np.nan
nd_bin_cdp[nd_all<10,:] = np.nan
nd_bin_2ds[nd_all<10,:] = np.nan
nd_m[nd_m<10] = np.nan
...
size_2ds_low = np.hstack((size_2ds[0:1]*0.5, size_2ds[0:-1]))
dlogDp_2ds = np.log10(size_2ds/size_2ds_low)
nd_bin_2ds = nd_bin_2ds/dlogDp_2ds
...
instruments = 'CDP-2DS'
size_nd = np.hstack((size_cdp, size_2ds))
nd_bin = np.hstack((nd_bin_cdp, nd_bin_2ds))
...
fig,ax = plot.hist([ccn2,ccn2_m,ccn2_m2],  weights=[w0,w1,w2], 
                    legend = ['Obs','E3SMv1','E3SMv2'], color=['k','r','b'], bins=np.arange(0,210,10), 
...
fig,ax = plot.hist([reff,reff_m,reff_m2],  weights=[w0,w1,w2], 
                    legend = ['Obs','E3SMv1','E3SMv2'], color=['k','r','b'], bins=np.arange(2,33,2), 
...
fig,ax = plot.percentile_z([ccn2,ccn2_m,ccn2_m2], [height,height,height], 
                      height_bin, figsize=(3,8), title='CCN (SS=0.2%)',color=['k','r','b'],
                      xlabel='cm$^{-3}$', ylabel='height (m)', legend = ['Obs', 'E3SMv1', 'E3SMv2'], )
fig.savefig(figpath+'percentile_z_ccn2_'+site+'.png',dpi=fig.dpi,bbox_inches='tight', pad_inches=1)
