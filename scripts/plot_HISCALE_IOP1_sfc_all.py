...
figpath= '/pscratch/sd/a/avarble/eagles/ESMAC_DIAG/figures/'+site+'/ne256mam/sfc_toa/'
...
obsdata.close()
org_hiscale = org.sel(time=obssfc_time_hiscale)
so4_hiscale = so4.sel(time=obssfc_time_hiscale)
nh4_hiscale = nh4.sel(time=obssfc_time_hiscale)
no3_hiscale = no3.sel(time=obssfc_time_hiscale)
chl_hiscale = chl.sel(time=obssfc_time_hiscale)
...
obsdata.close()
precip_tbrg_hiscale = precip_tbrg.sel(time=obssfc_time_hiscale)
precip_pwd_hiscale = precip_pwd.sel(time=obssfc_time_hiscale)
precip_pars_hiscale = precip_pars.sel(time=obssfc_time_hiscale)
...
obsdata.close()
lwdnsfc_hiscale = lwdnsfc.sel(time=obssfc_time_hiscale)
swdnsfc_hiscale = swdnsfc.sel(time=obssfc_time_hiscale)
lwupsfc_hiscale = lwupsfc.sel(time=obssfc_time_hiscale)
swupsfc_hiscale = swupsfc.sel(time=obssfc_time_hiscale)
...
obsdata.close()
uhsas100_hiscale = uhsas100.sel(time=obssfc_time_hiscale)
uhsasall_hiscale = uhsas_all.sel(time=obssfc_time_hiscale)
...
smps100_hiscale = smps100.sel(time=obssfc_time_hiscale)
smpsall_hiscale = smps_all.sel(time=obssfc_time_hiscale)
...
obsdata.close()
cth_hiscale = cth.sel(time=obssfc_time_hiscale)
cbh_hiscale = cbh.sel(time=obssfc_time_hiscale)

# satellite data
# obsdata = xr.open_dataset(prep_sat_path + 'albedo_VISSTgrid_'+site+'.nc')
obsdata = xr.open_dataset(prep_sat_path + 'albedo_VISSTpix_'+site+'.nc')
albedo_sat = obsdata['albedo'].load()
obsdata.close()
albedo_hiscale = albedo_sat.sel(time=obssat_time_hiscale)
...
obsdata.close()
cfall_sat_hiscale = cfall_sat.sel(time=obssat_time_hiscale)
# obsdata = xr.open_dataset(prep_sat_path + 'cloudtop_VISSTgrid_'+site+'.nc')
obsdata = xr.open_dataset(prep_sat_path + 'cloudtop_VISSTpix_'+site+'.nc')
ctt_sat = obsdata['ctt'].load()
cth_sat = obsdata['cth'].load()
obsdata.close()
ctt_sat_hiscale = ctt_sat.sel(time=obssat_time_hiscale)
cth_sat_hiscale = cth_sat.sel(time=obssat_time_hiscale)
# obsdata = xr.open_dataset(prep_sat_path + 'cod_VISSTgrid_'+site+'.nc')
obsdata = xr.open_dataset(prep_sat_path + 'cod_VISSTpix_'+site+'.nc')
cod_sat = obsdata['cod'].load()
obsdata.close()
cod_sat_hiscale = cod_sat.sel(time=obssat_time_hiscale)
# obsdata = xr.open_dataset(prep_sat_path + 'Reff_VISSTgrid_'+site+'.nc')
obsdata = xr.open_dataset(prep_sat_path + 'Reff_VISSTpix_'+site+'.nc')
reff_sat = obsdata['reff'].load()
obsdata.close()
reff_sat_hiscale = reff_sat.sel(time=obssat_time_hiscale)
# obsdata = xr.open_dataset(prep_sat_path + 'LWP_VISSTgrid_'+site+'.nc')
obsdata = xr.open_dataset(prep_sat_path + 'LWP_VISSTpix_'+site+'.nc')
lwp_sat = obsdata['lwp'].load()
obsdata.close()
lwp_sat_hiscale = lwp_sat.sel(time=obssat_time_hiscale)
# obsdata = xr.open_dataset(prep_sat_path + 'Nd_VISSTgrid_'+site+'.nc')
obsdata = xr.open_dataset(prep_sat_path + 'Nd_VISSTpix_'+site+'.nc')
nd_sat = obsdata['Nd'].load()
obsdata.close()
nd_sat_hiscale = nd_sat.sel(time=obssat_time_hiscale)
# obsdata = xr.open_dataset(prep_sat_path + 'lwflx_VISSTgrid_'+site+'.nc')
obsdata = xr.open_dataset(prep_sat_path + 'lwflx_VISSTpix_'+site+'.nc')
lwnettoa = obsdata['lwnettoa'].load()
obsdata.close()
lwnettoa_hiscale = lwnettoa.sel(time=obssat_time_hiscale)
# obsdata = xr.open_dataset(prep_sat_path + 'swflx_VISSTgrid_'+site+'.nc')
obsdata = xr.open_dataset(prep_sat_path + 'swflx_VISSTpix_'+site+'.nc')
swnettoa = obsdata['swnettoa'].load()
obsdata.close()
swnettoa_hiscale = swnettoa.sel(time=obssat_time_hiscale)
...
if config['netradiation_output'] == True:
            ...
            swnetsfc_m = modeldata[config['SWNETSFC']].load()
            swnettoa_m = modeldata[config['SWNETTOA']].load()
else:
            lwupsfc_m = modeldata[config['LWUPSFC']].load()
            swupsfc_m = modeldata[config['SWUPSFC']].load()
...
if config['tau3d_output'] == True:
            ...
            ndarm_m_hiscale = ndarm_m.sel(time=model_time_hiscale)
if config['reff_output'] == True:
            reff_m_hiscale = reff_m.sel(time=model_time_hiscale)
if config['cosp_output'] == True:
            lwp_modis_m_hiscale = lwp_modis_m.sel(time=model_time_hiscale)
            iwp_modis_m_hiscale = iwp_modis_m.sel(time=model_time_hiscale)
            reff_modis_m_hiscale = reff_modis_m.sel(time=model_time_hiscale)
            cod_modis_m_hiscale = cod_modis_m.sel(time=model_time_hiscale)
            cfliq_modis_m_hiscale = cfliq_modis_m.sel(time=model_time_hiscale)
            cftot_modis_m_hiscale = cftot_modis_m.sel(time=model_time_hiscale)
            nd_modis_m_hiscale = nd_modis_m.sel(time=model_time_hiscale)
            ndarm_m_hiscale = ndarm_m.sel(time=model_time_hiscale)
lwp_m_hiscale = lwp_m.sel(time=model_time_hiscale)
nd_m_hiscale = nd_m.sel(time=model_time_hiscale)
precip_m_hiscale = precip_m.sel(time=model_time_hiscale)
cld_m_hiscale = cld_m.sel(time=model_time_hiscale)
...
swnettoa_m_hiscale = swnettoa_m.sel(time=model_time_hiscale)
albedo_m_hiscale = albedo_m.sel(time=model_time_hiscale)
...
# bc_m2_hiscale = bc_m2.sel(time=model_time_hiscale)
# dst_m2_hiscale = dst_m2.sel(time=model_time_hiscale)
# org_m2_hiscale = org_m2.sel(time=model_time_hiscale)
# so4_m2_hiscale = so4_m2.sel(time=model_time_hiscale)
# ncl_m2_hiscale = ncl_m2.sel(time=model_time_hiscale)
# ccn2_m2_hiscale = ccn2_m2.sel(time=model_time_hiscale)
# ncn3_m2_hiscale = ncn3_m2.sel(time=model_time_hiscale)
# ncn10_m2_hiscale = ncn10_m2.sel(time=model_time_hiscale)
# ncn100_m2_hiscale = ncn100_m2.sel(time=model_time_hiscale)
# CNsize_m2_hiscale = CNsize_m2.sel(time=model_time_hiscale)
# cod_m2_hiscale = cod_m2.sel(time=model_time_hiscale)
# reff_m2_hiscale = reff_m2.sel(time=model_time_hiscale)
# lwp_m2_hiscale = lwp_m2.sel(time=model_time_hiscale)
# nd_m2_hiscale = nd_m2.sel(time=model_time_hiscale)
# precip_m2_hiscale = precip_m2.sel(time=model_time_hiscale)
# cld_m2_hiscale = cld_m2.sel(time=model_time_hiscale)
# cbh_m2_hiscale = cbh_m2.sel(time=model_time_hiscale)
# cth_m2_hiscale = cth_m2.sel(time=model_time_hiscale)
# Hcld_m2_hiscale = Hcld_m2.sel(time=model_time_hiscale)
# cldlow_m2_hiscale = cldlow_m2.sel(time=model_time_hiscale)
# cldmid_m2_hiscale = cldmid_m2.sel(time=model_time_hiscale)
# cldhgh_m2_hiscale = cldhgh_m2.sel(time=model_time_hiscale)
# lwnetsfc_m2_hiscale = lwnetsfc_m2.sel(time=model_time_hiscale)
# lwnettoa_m2_hiscale = lwnettoa_m2.sel(time=model_time_hiscale)
# swnetsfc_m2_hiscale = swnetsfc_m2.sel(time=model_time_hiscale)
# swnettoa_m2_hiscale = swnettoa_m2.sel(time=model_time_hiscale)
# albedo_m2_hiscale = albedo_m2.sel(time=model_time_hiscale)
...
modeldata.close()
cloud_m_hiscale = cf_e3sm.sel(time=model_time_hiscale)
...
# modeldata.close()
# cloud_m2_hiscale = cf_e3sm2.sel(time=model_time_hiscale)
