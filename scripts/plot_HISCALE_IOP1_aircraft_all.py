...
# read in E3SM data
lst = sorted(glob.glob(prep_model_path + 'HISCALE_flight_*.nc'))
...
if config['ccn_output'] == True:
    ...
    ccn2_m = modeldata['CCN2'].load().where(np.logical_and(modeldata.time>time1, modeldata.time<time2), drop=True)
    ccn5_m = modeldata['CCN5'].load().where(np.logical_and(modeldata.time>time1, modeldata.time<time2), drop=True)    
cwc_m = modeldata['cwc'].load().where(np.logical_and(modeldata.time>time1, modeldata.time<time2), drop=True)
if config['rain_output'] == True:
    rwc_m = modeldata['rwc'].load().where(np.logical_and(modeldata.time>time1, modeldata.time<time2), drop=True)
    lwc_m = cwc_m + rwc_m
else:
    lwc_m = cwc_m
...
pres_m = modeldata[config['p_mid']].load().where(np.logical_and(modeldata.time>time1, modeldata.time<time2), drop=True)
T_m = modeldata[config['T_mid']].load().where(np.logical_and(modeldata.time>time1, modeldata.time<time2), drop=True)
...
# ccn2_m2 = modeldata['CCN3'].load().where(np.logical_and(modeldata.time>time1, modeldata.time<time2), drop=True)
...
if config['aerosol_output'] == True:
    ...
    #%% mean aerosol size distribution
    fig,ax = plot.mean_size_witherror([size_beasd, np.arange(1,3001), np.arange(1,3001)], 
                      ...
                      # marker=['.',None,None], linestyles=['none','-','-'],
                            [beasd,ncn_m], 
                          legend = ['BEASD','Model'],color=['k','r'],
                          marker=['.',None], linestyles=['none','-'],
...
if config['dsd_output'] == True:
    ...
    fig,ax = plot.mean_size_witherror([size_nd, np.arange(1,1000), np.arange(1,1000)], 
                      ...
                      # legend = ['MergedSD','E3SMv1', 'E3SMv2'],color=['k','r','b'], xlimit=(1, 1e3), ylimit=(1e-5,1e3), 
                            [nd_size,nd_bin_m], 
                            marker=['.',None], linestyles=['none','-'],
                            # marker=['.','.'], linestyles=['none','none'],
                      legend = ['MergedSD','Model'],color=['k','r'], xlimit=(1, 1e3), ylimit=(1e-5,1e3), 
...
if config['aerosol_output'] == True:
    ...
    fig,ax = plot.hist([org,org_m], weights=[w0,w1], bins=np.arange(0,6.2,0.2),color=['k','r'],
                              legend = ['Obs','Model'], title='Total Organic '+site+' '+IOP, 
    ...
    fig,ax = plot.hist([so4,so4_m], weights=[w0,w1], bins=np.arange(0,5.2,0.2),color=['k','r'],
                              legend = ['Obs','Model'],title='Sulfate '+site+' '+IOP, 
                              ylabel='Fraction', xlabel='${\mu}$g/m$^{3}$')
    ...
    fig,ax = plot.hist([cpc3,ncn3_m], weights=[w0,w1], bins=np.arange(0,29000,1000), legend = ['Obs','Model'], 
                            color=['k','r'],title='Aerosol number (>3nm) '+site+' '+IOP, ylabel='Fraction', xlabel='cm$^{-3}$')
    ...
    fig,ax = plot.hist([cpc10,ncn10_m], weights=[w0,w1], bins=np.arange(0,22000,1000), legend = ['Obs','Model'], 
                            color=['k','r'], title='Aerosol number (>10nm) '+site+' '+IOP,ylabel='Fraction', xlabel='cm$^{-3}$')
...
fig,ax = plot.hist([lwc, lwc_m], weights=[w0,w1], bins=np.arange(0.01,0.5,0.02), legend = ['Obs','Model'], 
                        color=['k','r'], title='lwc '+site+' '+IOP, ylabel='Fraction', xlabel=lwc.units)
# ax.set_yscale('log')
# ax.set_ylim(1e-4,1)
...
fig,ax = plot.hist([nd, nd_m], weights=[w0, w1], bins=np.arange(1,510,20), legend = ['Obs','Model'], 
                        color=['k','r'], title='Nd '+site+' '+IOP, ylabel='Fraction', xlabel='cm$^{-3}$')
# ax.set_yscale('log')
# ax.set_ylim(1e-4,.1)
...
if config['reff_output'] == True:
    ...
    fig,ax = plot.hist([reff, reff_m], weights=[w0, w1], bins=np.arange(3,30,1), legend = ['Obs','Model'], 
                            color=['k','r'], title='Cloud Effective Radius '+site+' '+IOP, ylabel='Fraction', xlabel='$\mu$m')
...
if config['aerosol_output'] == True:
    ...
    fig,ax = plot.percentile_z([org,org_m], [height,height,height], 
                          height_bin, figsize=(3,8), title='Organic',color=['k','r'],
    ...
    # fig,ax = plot.percentile_z([so4, so4_m, so4_m2], [height,height,height,], 
                          ...
                          # xlabel='$\mu$g/m$^3$', ylabel='height (m)', legend = ['Obs', 'E3SMv1', 'E3SMv2'], )
    fig,ax = plot.percentile_z([so4, so4_m], [height,height,height,], 
                          height_bin, figsize=(3,8), title='Sulfate', color=['k','r'],
                          xlabel='$\mu$g/m$^3$', ylabel='height (m)', legend = ['Obs', 'Model'], )
    ...
    fig,ax = plot.percentile_z([cpc3,ncn3_m], [height,height,height], 
                          height_bin, figsize=(3,8), title='CN (>3nm)',color=['k','r'],
    ...
    # fig,ax = plot.percentile_z([cpc10,ncn10_m,ncn10_m2], [height,height,height], 
                          ...
                          # xlabel='cm$^{-3}$', ylabel='height (m)', legend = ['Obs', 'E3SMv1', 'E3SMv2'], )
    fig,ax = plot.percentile_z([cpc10,ncn10_m], [height,height,height], 
                          height_bin, figsize=(3,8), title='CN (>10nm)',color=['k','r'],
                          xlabel='cm$^{-3}$', ylabel='height (m)', legend = ['Obs', 'Model'], )
    ...
    # fig,ax = plot.percentile_z([pcasp100, ncn100_m, ncn100_m2], [height,height,height], 
                          ...
                          # xlabel='cm$^{-3}$', ylabel='height (m)', legend = ['Obs', 'E3SMv1', 'E3SMv2'], )
    fig,ax = plot.percentile_z([pcasp100, ncn100_m], [height,height,height], 
...
fig,ax = plot.percentile_z([lwc, lwc_m], [height,height,height], 
                      height_bin, figsize=(3,8), title='LWC',color=['k','r'],
                      xlabel='g/m$^{3}$', ylabel='height (m)', legend = ['Obs', 'Model'], )
...
# fig,ax = plot.percentile_z([nd, nd_m, nd_m2], [height,height,height], 
                      ...
                      # xlabel='cm$^{-3}$', ylabel='height (m)', legend = ['Obs', 'E3SMv1', 'E3SMv2'], )
fig,ax = plot.percentile_z([nd, nd_m], [height,height,height], 
                      height_bin, figsize=(3,8), title='Nd',color=['k','r'],
                      xlabel='cm$^{-3}$', ylabel='height (m)', legend = ['Obs', 'Model'], )
...
if config['reff_output'] == True:
    ...
    fig,ax = plot.percentile_z([reff, reff_m], [height,height,height], 
                          height_bin, figsize=(3,8), title='Reff',color=['k','r'],
                          xlabel='$\mu$m', ylabel='height (m)', legend = ['Obs', 'Model'], )
...
if config['ccn_output'] == True:
    ...
    fig,ax = plot.jointhist([pcasp100, ncn100_m],[ccn2.data, ccn2_m.data],
                        xedges=np.arange(0,500,20), yedges=np.arange(0,500,20), vmax=0.3,
                        normalize_x=True, #xlimit=(0,1000), ylimit=(0,1000),
                        xlabel=['CN (>100nm) (cm$^{-3}$)','CN (>100nm) (cm$^{-3}$)'], 
                        ylabel='0.2%CCN (cm$^{-3}$)', title=['Obs','Model'], )
    ...
    fig,ax = plot.jointhist([ccn2.data, ccn2_m.data], [nd.data, nd_m.data],
...
fig,ax = plot.jointhist( [nd.data, nd_m.data],[lwc.data*1000, lwc_m.data*1000], 
                    xedges=np.arange(0,500,50), yedges=np.arange(0,500,50), 
                    normalize_x=True, #xlimit=(0,1000), ylimit=(0,1000),
                    xlabel=['Nd (cm$^{-3}$)','Nd (cm$^{-3}$)'], 
                    ylabel='LWC (mg/m$^{3}$)', title=['Obs','Model'], )
...
if config['reff_output'] == True:
    ...
    fig,ax = plot.jointhist([reff, reff_m],[lwc, lwc_m],
                        xedges=np.arange(0,30,2), yedges=np.arange(0,0.6,0.05), vmax=0.3,
                        normalize_x=True, #xlimit=(0,1000), ylimit=(0,1000),
                        xlabel=['Reff ($\mu$m)','Reff ($\mu$m)'], 
                        ylabel='LWC (g/m3)', title=['Obs','Model'], )
...
if config['ccn_output'] == True:
    ...
    fig,ax = plot.scatter([ccn2.data, ccn2_m.data], [nd.data, nd_m.data],
                          title=['Obs','Model'], xlimit=(0,800), ylimit=(0,500),
...
fig,ax = plot.scatter([nd.data, nd_m.data], [lwc.data*1000, lwc_m.data*1000], 
                      title=['Obs','Model'], xlimit=(0,500), ylimit=(0,500),
...
if config['reff_output'] == True:
    ...
    fig,ax = plot.scatter([nd.data, nd_m.data], [reff.data, reff_m.data], 
                          title=['Obs','Model'], xlimit=(0,500), ylimit=(0,50),
