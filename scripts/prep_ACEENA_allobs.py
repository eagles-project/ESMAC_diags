...
beasdpath = '../raw_data/obs/ACEENA/aircraft/beasd/'
...
arsclbndpath = '../raw_data/obs/ACEENA/profile/arsclbnd/'
...
# air.prep_beasd(beasdpath,iwgpath, prep_data_path+'flight/', dt=60)               # merged aerosol size distribution
...
# prepare surface data. output time in 5-min (dt=300s) resolution
print('prepare surface data:')
sfc.prep_ACSM(acsmpath, prep_data_path+'surface/', dt=300)            # aerosol composition
sfc.prep_ccn(ccnsfcpath, prep_data_path+'surface/', dt=300)              # CCN number concentration
sfc.prep_CPC(cpcpath, prep_data_path+'surface/', dt=300)    # aerosol number concentration (>3 or 10nm)
sfc.prep_CPC_withENAmask(aerosolmaskpath, prep_data_path+'surface/', dt=300)    # aerosol mask data
sfc.prep_CNsize_UHSAS(uhsaspath, prep_data_path+'surface/', dt=300)   # aerosol size distribution from UHSAS
sfc.prep_cloud_2d(armbepath, prep_data_path+'surface/', height_out, dt=300)   # 2D cloud fraction
sfc.prep_cloudheight_ARSCL(arsclpath, prep_data_path+'surface/', dt=300)   # cloud height 
sfc.prep_totcld(armbepath, prep_data_path+'surface/', dt=300)         # cloud fraction. from ARSCL, TSI and satellite sources
sfc.prep_LWP(armbepath, mfrsrpath, prep_data_path+'surface/', dt=300) # cloud liquid water path
sfc.prep_Ndrop(ndroppath, prep_data_path+'surface/', dt=300)          # cloud droplet number retrieval from ARM Ndrop VAP
sfc.prep_Nd_WU(Wuetalpath, prep_data_path+'surface/', dt=300)           # cloud droplet number retrieval from Wu et al. algorithm
sfc.prep_mfrsr_cod(mfrsrpath,  prep_data_path+'surface/', dt=300)     # cloud optical depth from MFRSR
sfc.prep_mfrsr_Reff(mfrsrpath,  prep_data_path+'surface/', dt=300)    # cloud effective radius from MFRSR
sfc.prep_radiation(armbepath, prep_data_path+'surface/', dt=300)      # surface radiation
sfc.prep_LTS(armbepath, prep_data_path+'surface/', dt=300)            # lower tropospheric stability
sfc.prep_precip(armbepath, prep_data_path+'surface/', dt=300)         # surface precipitation
