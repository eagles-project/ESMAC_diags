...
if campaign=='MAGIC':
    ...
    print('ERROR: filename at this site is not specified.')
    error
lst.sort()
...
for filename in lst:
    ...
    for mm in range(len(Model_List)):
        model=Model_List[mm]
        variable1d_names = ['PS','PBLH','FLNT','FSNT','FLNS','FSNS',"LHFLX", "SHFLX", 'TREFHT','PRECT','PRECL',
                             "TGCLDLWP", "TGCLDIWP"]
        ...
        for dd in range(len(ymd)):
            ...
            (timem,var1d,timeunitm,var1dunit,var1dlongname)=read_E3SM(filename_input,[a+'_'+E3SMdomain_range for a in variable1d_names])
            (timem,var2d,timeunitm,var2dunit,var2dlongname)=read_E3SM(filename_input,[a+'_'+E3SMdomain_range for a in variable2d_names])
