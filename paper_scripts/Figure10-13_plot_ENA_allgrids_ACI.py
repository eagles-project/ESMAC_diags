...
#%%
# fig,ax = plot.jointhist([ndrop.data[idx_sfc],nd_wu.data[idx_sfc], nd_sat[idx_sfc,5,5].data, nd_m.data[idx_m1].flatten(),nd_visst_m.data[idx_m1].flatten(),], 
#                         [lwp_sat[idx_sfc,5,5].data,lwp[idx_sfc].data, lwp[idx_sfc].data, lwp_m.data[idx_m1].flatten(),lwp_m.data[idx_m1].flatten(),], 
#                     xedges=np.exp(np.arange(np.log(10),7,0.2)), yedges=np.exp(np.arange(np.log(10),7,0.2)), 
#                     normalize_x=True, vmin=0, vmax=0.15,
#                     xlabel=['Nd (cm$^{-3}$)','Nd (cm$^{-3}$)','Nd (cm$^{-3}$)','Nd (cm$^{-3}$)','Nd (cm$^{-3}$)',], 
#                     ylabel='LWP (g/m$^2$)', title=['Ground','Wu_etal','Satellite','E3SMv2','E3SMv2-sat'], )
# for ax0 in ax[0,:]:
#     ax0.set_xscale('log')
#     ax0.set_yscale('log')
#     # ax0.set_ylim(10,600)
...
#     ax0.set_xticklabels([10,30,100,300])
# regress,sample = calc.linear_regress(
#         [np.log(ndrop[idx_sfc].data), np.log(nd_wu[idx_sfc].data), np.log(nd_sat.data[idx_sat].flatten()), np.log(nd_m.data[idx_m1].flatten()), np.log(nd_visst_m.data[idx_m1].flatten()), ],
#         [np.log(lwp.data[idx_sfc]),np.log(lwp.data[idx_sfc]), np.log(lwp_sat.data[idx_sat].flatten()), np.log(lwp_m.data[idx_m1].flatten()), np.log(lwp_m.data[idx_m1].flatten())], 
#         figpath+'linearfit_LWP_Nd_sfccouple_allgrids.txt',legend=['Surface','Wu_etal','Satellite','E3SMv2','E3SMv2-sat'],
#         labelx='ln(Nd)',labely='ln(LWP)')
# xdata = [ndrop.data[idx_sfc], nd_wu.data[idx_sfc], nd_sat.data[idx_sat].flatten(), nd_m.data[idx_m1].flatten(),nd_visst_m.data[idx_m1].flatten(),]
# for nn in range(5):
#     x = [np.nanmin(xdata[nn]), min(np.nanmax(xdata[nn]),1000)]
#     y = np.exp(regress[nn][0] * np.log(x) + regress[nn][1])
#     ax[0,nn].plot(x, y, color='r')
