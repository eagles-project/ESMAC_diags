...
wt_o = np.ones_like(cpc_o)/np.count_nonzero(~np.isnan(cpc_o))
wt_m = np.ones_like(cpc_m)/np.count_nonzero(~np.isnan(cpc_m))
ax2.hist([cpc_o, cpc_m], bins=np.arange(21)*100, histtype='bar', weights=[wt_o, wt_m],color=['r','g'])
...
ax3.plot(acsmtime,org_o,color='r',marker='.')
ax3.plot(acsmtime,org_m,color='green',marker='.')
...
wt_o = np.ones_like(org_o)/np.count_nonzero(~np.isnan(org_o))
wt_m = np.ones_like(org_m)/np.count_nonzero(~np.isnan(org_m))
ax4.hist([org_o, org_m], bins=np.arange(21)*0.075, histtype='bar', weights=[wt_o, wt_m],color=['r','g'])
