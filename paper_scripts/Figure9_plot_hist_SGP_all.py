...
fig,ax = plot.hist([ccn2,ccn2_m], weights=[w1,w2], bins=np.arange(0,1500,50),
                    ...
                    xlabel = '(a) Surface CCN (SS=0.2%) (cm$^{-3}$)', ylabel='Fraction')
                    # title = '(a) Surface CCN (SS=0.2%) '+site, ylabel='Fraction', xlabel='cm$^{-3}$')
...
fig,ax = plot.hist([ndrop[idx_sfc],nd_sat[idx_sat],nd_m[idx_m]],  weights=[w0,w000,w1], 
                   legend = ['Ndrop','Satellite', 'E3SMv2'], color=['k','gray','r'],bins=np.arange(0,410,20), 
                   xlabel = '(c) Nd (cm$^{-3}$)', ylabel='Fraction')
                    # title = '(c) Nd '+site, ylabel='Fraction', xlabel='cm$^{-3}$')
...
fig,ax = plot.hist([reff[idx_sfc],reff_sat[idx_sat],reff_m[idx_m]], weights=[w0,w000,w1], 
                    legend = ['MFRSR','Satellite', 'E3SMv2'], color=['k','gray','r'], bins=np.arange(3,22,1), 
                    xlabel = '(e) Cloud Effective Radius ($\mu$m)', ylabel='Fraction')
                    # title = '(e) Cloud Effective Radius '+site,ylabel='Fraction', xlabel='$\mu$m')
...
fig,ax = plot.hist( [cod[idx_sfc], cod_sat[idx_sat], cod_m[idx_m], ], weights=[w0,w00,w1,], 
                    legend = ['MFRSR','Satellite','E3SMv2',], color=['k','gray','r',],bins=np.arange(2,81,3), 
                    xlabel = '(g) Cloud Optical Depth (unitless)', ylabel='Fraction')
                    # title='(g) Cloud Optical Depth '+site, ylabel='Fraction', xlabel='unitless')
...
fig,ax = plot.hist([cld_arscl,cld_visst,cld_m], 
                    ...
                    xlabel = '(i) Cloud Fraction (%)', ylabel='Fraction')
                      # title = '(i) Cloud Fraction '+site, ylabel='Fraction', xlabel="%")
