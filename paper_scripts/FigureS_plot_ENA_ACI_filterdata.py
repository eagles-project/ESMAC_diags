...
# idx_sfc = np.logical_and(np.logical_and(ctt_sat>273, cld_armbe>90), thetadiff_cb<2)
# idx_sat = np.logical_and(np.logical_and(ctt_sat>273, cfall_sat>90), thetadiff_cb<2)
# idx_m1 = np.logical_and(np.logical_and(ctt_m>273, cld_m>90), thetadiff_cb_m<2)
...
#%%
# w2 = np.ones_like(thetadiff_cb[:])/sum(~np.isnan(thetadiff_cb[:].data))
# w21 = np.ones_like(thetadiff_cb_m[:])/sum(~np.isnan(thetadiff_cb_m[:].data))
# fig,ax = plot.hist([thetadiff_cb[:],thetadiff_cb_m[:],], 
#                     weights=[w2,w21,], bins=np.arange(0,41,1), 
#                     legend = ['obs','E3SMv2',], color=['k','r',],
#                     title = 'Thetadiff '+site, ylabel='Fraction', xlabel='cm$^{-3}$')
...
regress,sample = calc.linear_regress([np.log(ccn1m_all[:].data), np.log(ccn1m_all[:].data), np.log(ccn1_m[:].data),], 
                        ...
                        figpath+'linearfit_Nd_CCN_ENAall.txt',legend=['Surface','Satellite','E3SMv2'],
...
regress,sample = calc.linear_regress([np.log(ndrop[:].data), np.log(nd_sat[:].data), np.log(nd_m[:].data)],
                        ...
                        figpath+'linearfit_LWP_Nd_ENAall.txt',legend=['Surface','Satellite','E3SMv2',],
...
regress,sample = calc.linear_regress([np.log(ccn1m_all[idx_sfc].data), np.log(ccn1m_all[idx_sat].data), np.log(ccn1_m[idx_m1].data),], 
                        ...
                        figpath+'linearfit_Nd_CCN_ENAfilter.txt',legend=['Surface','Satellite','E3SMv2'],
                        labelx='ln(CCN1)',labely='ln(Nd)')
...
regress,sample = calc.linear_regress([np.log(ndrop[idx_sfc].data), np.log(nd_sat[idx_sat].data), np.log(nd_m[idx_m1].data)],
                        ...
                        figpath+'linearfit_LWP_Nd_ENAfilter.txt',legend=['Surface','Satellite','E3SMv2',],
                        labelx='ln(Nd)',labely='ln(LWP)')
