...
iwp_sat = visstdata['water_path'][:,y_idx,x_idx,0]
...
for ii in range(1,len(lst)):
    ...
    iwp_sat = xr.concat([iwp_sat, visstdata['water_path'][:,y_idx,x_idx,0]], dim="time")
...
# surface: 1, satellite: 0
color_3 = ['k', CB_color_cycle[1], CB_color_cycle[0]]
...
fig,ax = plot.hist([nd_air_5, nd_sfc_5, nd_sat_5], weights=[w0, w1, w2], bins=np.arange(0,810,40), 
                   legend = ['aircraft ('+format(1/w0[0],'.0f')+')','Ndrop ('+format(1/w1[0],'.0f')+')','VISST ('+format(1/w2[0],'.0f')+')'], 
                   ylabel='Fraction', xlabel='cm$^{-3}$', color=color_3, ylimit=(0,0.4), 
