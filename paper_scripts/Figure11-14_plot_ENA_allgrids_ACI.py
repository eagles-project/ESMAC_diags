...
ccn1_all = ccn1m.interp(time=time_cpc)
...
iwp_sat.data[zenith_sat.data>70]=np.nan
...
# idx_sfc = np.logical_and(ctt_sat[:,y0,x0]>273, cld_armbe>90)
# idx_sat = np.logical_and(ctt_sat.data>273, cfall_sat.data>90)
# idx_m1 = np.logical_and(ctt_m.data>273, cldlow_m.data>90)
...
idx_sfc[zenith_sat[:,y0,x0]>70] = False
idx_m1[zenith_sat[:,y0,x0]>70,:] = False
...
# fig,ax = plot.heatmap([ndrop.data[idx_sfc], nd_sat.data[idx_sat].flatten(), nd_m.data[idx_m1].flatten(),], 
#                       [lwp[idx_sfc].data, lwp_sat.data[idx_sat].flatten(), lwp_m.data[idx_m1].flatten(), ], 
#                       [albedo_sat.data[idx_sfc,y0,x0], albedo_sat.data[idx_sat].flatten(), albedo_m.data[idx_m1].flatten(), ], 
...
# idx = np.logical_and(np.logical_and(~np.isnan(nd_wu.data), ~np.isnan(lwp.data)), ~np.isnan(albedo_sat.data[:,7,9]))
# y = albedo_sat[idx,7,9].data
# x = np.log(np.vstack((nd_wu[idx].data, lwp[idx].data)).T)
# X = sm.add_constant(x)
# sfc = sm.OLS(y, X).fit()

# idx = np.logical_and(np.logical_and(~np.isnan(nd_sat.data), ~np.isnan(lwp_sat.data)), ~np.isnan(albedo_sat.data))
# y = albedo_sat.data[idx]
# x = np.log(np.vstack((nd_sat.data[idx], lwp_sat.data[idx])).T)
# X = sm.add_constant(x)
# sat = sm.OLS(y, X).fit()

# idx = np.logical_and(np.logical_and(~np.isnan(nd_m.data), ~np.isnan(lwp_m.data)), ~np.isnan(albedo_m.data))
# y = albedo_m.data[idx]
# x = np.log(np.vstack((nd_m.data[idx], lwp_m.data[idx])).T)
# X = sm.add_constant(x)
# e3sm1 = sm.OLS(y, X).fit()

# idx = np.logical_and(np.logical_and(~np.isnan(nd_m2.data), ~np.isnan(lwp_m2.data)), ~np.isnan(albedo_m2.data))
# y = albedo_m2.data[idx]
# x = np.log(np.vstack((nd_m2.data[idx], lwp_m2.data[idx])).T)
# X = sm.add_constant(x)
# e3sm2 = sm.OLS(y, X).fit()

# idx = np.logical_and(np.logical_and(~np.isnan(nd_arm_m.data), ~np.isnan(lwp_m.data)), ~np.isnan(albedo_m.data))
# y = albedo_m.data[idx]
# x = np.log(np.vstack((nd_arm_m.data[idx], lwp_m.data[idx])).T)
# X = sm.add_constant(x)
# e3sm1_arm = sm.OLS(y, X).fit()

# idx = np.logical_and(np.logical_and(~np.isnan(nd_arm_m2.data), ~np.isnan(lwp_m2.data)), ~np.isnan(albedo_m2.data))
# y = albedo_m2.data[idx]
# x = np.log(np.vstack((nd_arm_m2.data[idx], lwp_m2.data[idx])).T)
# X = sm.add_constant(x)
# e3sm2_arm = sm.OLS(y, X).fit()

# idx = np.logical_and(np.logical_and(~np.isnan(nd_visst_m.data), ~np.isnan(lwp_m.data)), ~np.isnan(albedo_m.data))
# y = albedo_m.data[idx]
# x = np.log(np.vstack((nd_visst_m.data[idx], lwp_m.data[idx])).T)
# X = sm.add_constant(x)
# e3sm1_sat = sm.OLS(y, X).fit()

# idx = np.logical_and(np.logical_and(~np.isnan(nd_visst_m2.data), ~np.isnan(lwp_m2.data)), ~np.isnan(albedo_m2.data))
# y = albedo_m2.data[idx]
# x = np.log(np.vstack((nd_visst_m2.data[idx], lwp_m2.data[idx])).T)
# X = sm.add_constant(x)
# e3sm2_sat = sm.OLS(y, X).fit()

#%% with satellite simulator

# # Nd vs surface CCN
# fig,ax = plot.jointhist([ccn1_all[idx_sfc].data, ccn_sat[idx_sat].flatten(), ccn1_m.data[idx_m1].flatten(), ccn1_m.data[idx_m1].flatten()], 
#                         [ndrop[idx_sfc].data, nd_sat.data[idx_sat].flatten(), nd_m.data[idx_m1].flatten(), nd_visst_m.data[idx_m1].flatten()], 
#                     xedges=np.exp(np.arange(np.log(10),7,0.2)), yedges=np.exp(np.arange(np.log(10),7,0.2)), 
#                     normalize_x=True, vmin=0, vmax=0.16,
#                     xlabel=['0.1%CCN (cm$^{-3}$)','0.1%CCN (cm$^{-3}$)','0.1%CCN (cm$^{-3}$)','0.1%CCN (cm$^{-3}$)'], 
#                     ylabel='Nd (cm$^{-3}$)', title=['Ground','Satellite','E3SMv2','E3SMv2-sat'], )
# for ax0 in ax[0,:]:
#     ax0.set_xscale('log')
#     ax0.set_yscale('log')
#     ax0.set_xticks([10,30,100,300,1000])
...
#     ax0.set_xticklabels([10,30,100,300,1000])
# regress,sample = calc.linear_regress(
#         [np.log(ccn1_all[idx_sfc].data), np.log(ccn_sat[idx_sat].flatten()), np.log(ccn1_m.data[idx_m1].flatten()),np.log(ccn1_m.data[idx_m1].flatten()),], 
#         [np.log(ndrop[idx_sfc].data), np.log(nd_sat.data[idx_sat].flatten()), np.log(nd_m.data[idx_m1].flatten()),np.log(nd_visst_m.data[idx_m1].flatten()), ],
#         figpath+'linearfit_Nd_CCN_sfccouple.txt',legend=['Surface','Satellite','E3SMv2','E3SMv2-sat',],
#         labelx='ln(CCN1)',labely='ln(Nd)')
# xdata = [ccn1_all[idx_sfc].data, ccn_sat[idx_sat].flatten(), ccn1_m.data[idx_m1].flatten(), ccn1_m.data[idx_m1].flatten()]
# for nn in range(4):
#     x = [np.nanmin(xdata[nn]), min(np.nanmax(xdata[nn]),1000)]
#     y = np.exp(regress[nn][0] * np.log(x) + regress[nn][1])
#     ax[0,nn].plot(x, y, color='r')
...
# #%%
# fig,ax = plot.jointhist([ndrop.data[idx_sfc], nd_sat.data[idx_sat].flatten(), nd_m.data[idx_m1].flatten(),nd_visst_m.data[idx_m1].flatten(),], 
#                         [lwp[idx_sfc].data, lwp_sat.data[idx_sat].flatten(), lwp_m.data[idx_m1].flatten(),lwp_m.data[idx_m1].flatten(),], 
#                     xedges=np.exp(np.arange(np.log(10),7,0.2)), yedges=np.exp(np.arange(np.log(10),7,0.2)), 
#                     normalize_x=True, vmin=0, vmax=0.15,
#                     xlabel=['Nd (cm$^{-3}$)','Nd (cm$^{-3}$)','Nd (cm$^{-3}$)','Nd (cm$^{-3}$)',], 
#                     ylabel='LWP (g/m$^2$)', title=['Ground','Satellite','E3SMv2','E3SMv2-sat'], )
# for ax0 in ax[0,:]:
#     ax0.set_xscale('log')
#     ax0.set_yscale('log')
#     # ax0.set_ylim(10,600)
...
# regress,sample = calc.linear_regress(
#         [np.log(ndrop[idx_sfc].data), np.log(nd_sat.data[idx_sat].flatten()), np.log(nd_m.data[idx_m1].flatten()), np.log(nd_visst_m.data[idx_m1].flatten()), ],
#         [np.log(lwp.data[idx_sfc]), np.log(lwp_sat.data[idx_sat].flatten()), np.log(lwp_m.data[idx_m1].flatten()), np.log(lwp_m.data[idx_m1].flatten())], 
#         figpath+'linearfit_LWP_Nd_sfccouple_allgrids.txt',legend=['Surface','Satellite','E3SMv2','E3SMv2-sat'],
#         labelx='ln(Nd)',labely='ln(LWP)')
# xdata = [ndrop.data[idx_sfc], nd_sat.data[idx_sat].flatten(), nd_m.data[idx_m1].flatten(),nd_visst_m.data[idx_m1].flatten(),]
# for nn in range(4):
#     x = [np.nanmin(xdata[nn]), min(np.nanmax(xdata[nn]),1000)]
#     y = np.exp(regress[nn][0] * np.log(x) + regress[nn][1])
#     ax[0,nn].plot(x, y, color='r')
...
# # idx_sat[cod_sat<3] = False
# # idx_m1[cod_m<3] = False
# xedges=np.exp(np.arange(np.log(10),7,0.5))
# yedges=np.exp(np.arange(np.log(10),7,0.5))
# fig,ax = plot.heatmap([nd_sat.data[idx_sat].flatten(), nd_m.data[idx_m1].flatten(), nd_visst_m.data[idx_m1].flatten(), ], 
#                       [lwp_sat.data[idx_sat].flatten(), lwp_m.data[idx_m1].flatten(),lwp_m.data[idx_m1].flatten(),], 
#                       [albedo_sat.data[idx_sat].flatten(), albedo_m.data[idx_m1].flatten(),albedo_m.data[idx_m1].flatten(),], 
#                     xedges=xedges, yedges=yedges, 
#                     vmin=20, vmax=60, cmap='plasma',min_sample=10,
#                     xlabel='Nd (cm$^{-3}$)', ylabel='LWP (g/m$^2$)', 
#                     # title=['Ground','Satellite','E3SMv2','E3SMv2'])
#                     title=['Satellite','E3SMv2','E3SMv2-sat'])
# fig.text(.91, .85,'TOA Albedo (%)')      # unit of color variable
# xticklabels = (xedges[0:-1] + xedges[1:])/2
# yticklabels = (yedges[0:-1] + yedges[1:])/2
# xticks = np.interp(np.array([10,30,100,300]),xticklabels,np.arange(len(xticklabels)))
